...
<task type="auto">
  <name>Wire GoalsProvider to StatisticsScreen for custom goal display</name>
  <files>push_up_5050/lib/screens/statistics/statistics_screen.dart</files>
  <action>
In push_up_5050/lib/screens/statistics/statistics_screen.dart, add GoalsProvider integration so custom goals display in the statistics UI.

Current state: Line 58 uses only `Consumer<UserStatsProvider>`. GoalsProvider is not connected.

Required changes:

1. Add GoalsProvider import at top:
```dart
import 'package:push_up_5050/providers/goals_provider.dart';
```

2. Modify the widget tree at line 56-66 to wrap the existing Consumer<UserStatsProvider> with Consumer<GoalsProvider>:

Pattern (nested Consumer approach):
```dart
...
3. Update _buildContent method signature to accept GoalsProvider:
```dart
Widget _buildContent(UserStatsProvider stats, GoalsProvider goals) {
```

4. Update TotalPushupsCard call (line 120) to use custom daily goal:
```dart
TotalPushupsCard(total: total, goal: goals.dailyGoal.target),
```
Where `goals.dailyGoal` is the Goal object with custom target from storage.

5. If displaying weekly/monthly goal cards elsewhere, use:
   - Weekly goal: `goals.weeklyGoal.target` (auto-calculated as daily * 7)
   - Monthly goal: `goals.monthlyGoal.target`

Key points:
- GoalsProvider must be in main.dart's MultiProvider list (verify if already present)
- Use nested Consumer pattern to access both providers without rebuilding entire tree
- dailyGoal, weeklyGoal, monthlyGoal are Goal objects with `.target` property containing the int value
- Fallback to defaults (50/350/1500) happens automatically in GoalsProvider._loadSavedGoals()
</action>
  <verify>flutter run -d windows then navigate to Statistics screen; custom daily goal target appears in TotalPushupsCard progress bar</verify>
  <done>StatisticsScreen imports GoalsProvider, uses nested Consumer pattern, passes goals.dailyGoal.target to TotalPushupsCard goal parameter</done>
</task>
...
3. StatisticsScreen integration:
   - Imports GoalsProvider
   - Uses nested Consumer<GoalsProvider> pattern
   - _buildContent accepts GoalsProvider parameter
   - TotalPushupsCard receives goals.dailyGoal.target as goal parameter
   - context.watch<GoalsProvider>() or Consumer<GoalsProvider> pattern present
